#########################################Plot4.R SCRIPT######################################
## THIS SCRIPTS PLOTS AND CREATES plot4.png  file. IN THIS SCRIPTS TWO(2) FUNCTIONS ARE    ##
## DEFINED AND USED:                                                                       ##
...
generatePlot <- function (filename) {
  ...
  plot2Data <- data.frame (DateTime =formattedDateTime, Global_active_power = targetData$Global_active_power)
  ...
  ## subset additional data for plot 4 
  plot4Data <- data.frame (DateTime =formattedDateTime, 
                           Voltage = as.numeric(as.character(targetData$Voltage)),
                           Global_reactive_power = as.numeric(as.character(targetData$Global_reactive_power)) )
  ...
  ## set panel of 2 rows and 2 columns
  par (mfrow=c(2,2) )
 
  ##draw top left plot
  plot(plot2Data$DateTime, plot2Data$Global_active_power, type="l", ylab = "Global Active Power (killowatts", xlab=" ")
  
  ##draw top right plot
  plot(plot4Data$DateTime, plot4Data$Voltage, type="l", ylab = "Voltage", xlab="DateTime")
  ...
  plot(plot4Data$DateTime, plot4Data$Global_reactive_power, type="l", ylab = "Global_reactive_power", xlab="DateTime")
